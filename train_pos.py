...
if __name__ == '__main__':
  ...
  print("Loading Data")
  command_map, command_invmap, action_map, action_invmap = pickle.load(
      open(args.mapFile, 'rb'))
  word_to_pos = {
      'walk': 'verb',
      'and': 'conjunction',
      'turn': 'verb',
      ...
      'after': 'conjunction',
      'run': 'verb',
      'end_command': 'boundary_token',
      'right': 'direction'
  }

  def build_map_invmap(uni_tokens):
    fmap = dict(zip(uni_tokens, range(len(uni_tokens))))
    invmap = dict(zip(range(len(uni_tokens)), uni_tokens))
    return fmap, invmap

  pos_map, pos_invmap = build_map_invmap(set(word_to_pos.values()))
  cmdidx2posidx_map = {
      cmdidx: pos_map[word_to_pos[command_invmap[cmdidx]]]
      for cmdidx in command_invmap.keys()
  }
  breakhere = 1
  trainset = m1.DataSet(
      args.trainFile,
  ...
  model_paras['num_pos'] = len(pos_map)
