...
def file2np(commands, actions, structures, command_map, action_map,
            max_cmd_len, max_actions_per_subprogram, max_num_subprograms):
  # Convert to np
  commands_ind = [[command_map[c]
                   for c in cmd] + [0] * (max_cmd_len - len(cmd))
                  for cmd in commands]
  ...
  for row in range(len(structures)):
    ...
    for step in struct:
      ...
      a = act[start:end]
      padding = max_actions_per_subprogram - step - 1  # Add end action to each sub-program
      action_row.append(a + [action_map['end_action']] + [0] * padding)
      start = end
    actions_structured.append(action_row +
                              [[action_map['end_subprogram']] + [0] *
                               (max_actions_per_subprogram - 1)] +
                              [[0] * max_actions_per_subprogram] *
                              (max_num_subprograms - len(struct) - 1))
  act_np = np.array(actions_structured)
  struct_padded = [
      [sa + 1 for sa in s] + [1] + [0] * (max_num_subprograms - len(s) - 1)
      for s in structures
  ]  # Add end
  struct_np = np.array(struct_padded)

  mask_list = [[
      np.concatenate((np.ones(st), np.zeros(max_actions_per_subprogram - st)),
                     0) for st in s
  ] for s in struct_np]
...
class DataSet(object):
  def __init__(self,
    ...
    self.pos_np = np.vectorize(cmdidx2posidx_map.get)(self.cmd_np)
  ...
  def next_batch(self, batch_size, isTrain=True):
    ...
    if batch_size >= self._dataSize:
      cmd, pos, act, mask, struct, cmd_length = self.cmd_np, self.pos_np, self.act_np, self.mask_np, self.struct_np, self.cmd_lengths_np
      idx = self._allIdx
    else:
      ...
      if (self._index_in_epoch == batch_size) & (
          self._epochs_completed == 0) & isTrain:
        # Re-Shuffle the data indices at initial epoch
        np.random.shuffle(self._perm)

      batch_idx = self._perm[start:self._index_in_epoch]
      cmd, pos, act, mask, struct, cmd_length = \
          self.cmd_np[batch_idx], self.pos_np[batch_idx], self.act_np[batch_idx], self.mask_np[batch_idx], \
          self.struct_np[batch_idx], self.cmd_lengths_np[batch_idx]

      idx = self._allIdx[batch_idx]

    return cmd, pos, act, mask, struct, cmd_length, idx
...
class m1():
  def __init__(self, model_paras):
    ...
    self.num_pos = model_paras.get('num_pos', 5)
    ...
    self.max_actions_per_subprogram = model_paras.get(
    ...
    self.hidden_filters_subprogram = model_paras.get(
        'hidden_filters_subprogram', 128)
    self.num_layers_subprogram = model_paras.get('num_layers_subprogram', 3)
    self.l2_lambda = model_paras.get('l2_lambda',
                                     1e-3)  # coefficient of L2 penalization
  ...
  def _buildGraph(self):
    ...
    self.pos_ind = tf.placeholder(tf.int32, shape=(None, self.max_cmd_len))
    self.act_ind = tf.placeholder(
        tf.int32,
        shape=(None, self.max_num_subprograms,
               self.max_actions_per_subprogram))
    self.mask_ph = tf.placeholder(
        tf.float32,
        shape=(None, self.max_num_subprograms,
               self.max_actions_per_subprogram))
    self.cmd_lengths = tf.placeholder(tf.int32, shape=(None, ))
    self.act_lengths = tf.placeholder(
        tf.int32, shape=(None, self.max_num_subprograms))
    self.learning_rate = tf.placeholder(tf.float32, shape=(None))

    ## Embeddings
    self.cmd_mat = tf.Variable(
        self.init_mag * tf.random_normal([self.num_cmd, self.size_emb]))
    self.act_mat = tf.Variable(
        self.init_mag * tf.random_normal([self.num_act, self.size_emb]))
    self.act_st_emb = tf.Variable(self.init_mag * tf.random_normal(
        [self.size_emb]))  # Start_action token and embedding
    cmd_emb = tf.nn.embedding_lookup(self.cmd_mat, self.cmd_ind)
    cmd_emb_flat = tf.reshape(cmd_emb, (-1, self.size_emb))
    self.pos_forecast_flat = tf.layers.dense(cmd_emb_flat, self.num_pos)
    self.pos_forecast = tf.reshape(self.pos_forecast_flat,
                                   (-1, self.max_cmd_len, self.num_pos))
    cmd_emb_wth_pos = tf.concat((cmd_emb, tf.nn.softmax(self.pos_forecast, 2)),
                                2)
    act_emb = tf.nn.embedding_lookup(self.act_mat, self.act_ind)
    tf_bs = tf.shape(self.act_ind)[0]
    act_st_emb_expanded = tf.tile(
        tf.reshape(self.act_st_emb, [1, 1, 1, self.size_emb]),
        [tf_bs, self.max_num_subprograms, 1, 1])
    act_emb_with_st = tf.concat((act_st_emb_expanded, act_emb), 2)

    ## Encoder
    first_cell_encoder = [
        tf.nn.rnn_cell.LSTMCell(
            self.hidden_filters, forget_bias=1., name='layer1_' + d)
        for d in ['f', 'b']
    ]
    if self.num_layers_encoder > 1:
      hidden_cells_encoder = [[
          tf.nn.rnn_cell.LSTMCell(
              self.hidden_filters,
              forget_bias=1.,
              name='layer' + str(lidx) + '_' + d) for d in ['f', 'b']
      ...
      hidden_cells_encoder = [[
          tf.nn.rnn_cell.DropoutWrapper(
              ...
              dtype=tf.float32) for cell in cells
                              ...
                              for cells in hidden_cells_encoder[:-1]] + [
                                  hidden_cells_encoder[-1]
    ...
    encoding_last_layer, encoding_final_cells, encoding_hidden_layers, encoding_last_timestep, dbg1, dbg2 = \
        encode(cmd_emb_wth_pos, self.num_layers_encoder, cells_encoder, None, lengths=self.cmd_lengths, name='encoder')
    hidden_filters_encoder = encoding_last_timestep.shape[-1].value

    ## Decoder
    first_cell_subprogram = tf.nn.rnn_cell.LSTMCell(
        self.hidden_filters_subprogram,
        forget_bias=1.,
        name='subpogramlayer1_')
    ...
    attention_mechanism = tf.contrib.seq2seq.LuongAttention(
        ...
        memory_sequence_length=self.cmd_lengths)
    cells_subprogram = [
        tf.contrib.seq2seq.AttentionWrapper(
            cell,
            attention_mechanism,
            attention_layer_size=self.hidden_filters_subprogram)
    ...
    for sub_idx in range(self.max_num_subprograms):
      from_last_layer = tf.tile(
          tf.expand_dims(
              tf.concat((initial_cmb_encoding, last_encoding), 1), 1),
          [1, self.max_actions_per_subprogram + 1, 1
           ])  # Shape bs x (self.max_actions_per_subprogram) x LSTM_outx2
      autoregressive = act_emb_with_st[:,
                                       sub_idx, :, :]  # Use previous action as input to next action
      x_input = tf.concat(
          (from_last_layer, autoregressive),
          -1)  # bs x (self.max_actions_per_subprogram) x (input dim)
      subprogram_last_layer, _, subprogram_hidden_layers, subprogram_output = subprogram(
          x_input,
          self.num_layers_subprogram,
          cells_subprogram,
          None,
          lengths=self.act_lengths[:, sub_idx],
          reuse=(sub_idx > 0),
          name='subprogram')  # Reuse the same network across subprogram
      action_prob_flat = util.mlp(
          tf.reshape(subprogram_last_layer,
                     [-1, self.hidden_filters_subprogram]), [],
          output_size=self.num_act,
          name='action_choice_mlp',
          reuse=(sub_idx > 0))  # (bs x num_act_sub) x num_act
      action_prob_expanded = tf.reshape(
          action_prob_flat,
          [-1, self.max_actions_per_subprogram + 1, self.num_act])
    ...
    act_presoftmax_flat = tf.reshape(
        act_presoftmax, [-1, self.max_actions_per_subprogram, self.num_act])
    mask_ph_flat = tf.reshape(self.mask_ph,
                              [-1, self.max_actions_per_subprogram])
    act_ind_flat = tf.reshape(self.act_ind,
                              [-1, self.max_actions_per_subprogram])
    ...
    pos_loss = tf.nn.softmax_cross_entropy_with_logits_v2(
        labels=tf.one_hot(self.pos_ind, self.num_pos),
        logits=self.pos_forecast)
    pos_loss_avg = tf.reduce_mean(tf.pow(pos_loss, 2.0))
    tfvars = tf.trainable_variables()
    weight_norm = tf.reduce_mean(
        [tf.reduce_sum(tf.square(var)) for var in tfvars]) * self.l2_lambda

    action_taken = tf.argmax(act_presoftmax, -1, output_type=tf.int32)
    correct_mat = tf.cast(tf.equal(action_taken, self.act_ind),
                          tf.float32) * self.mask_ph
    correct_percent = tf.reduce_sum(correct_mat, [1, 2]) / tf.reduce_sum(
        self.mask_ph, [1, 2])
    self.percent_correct = tf.reduce_mean(correct_percent)
    self.percent_fully_correct = tf.reduce_mean(
        tf.cast(tf.equal(correct_percent, 1), tf.float32))

    self.loss = ppl_loss_avg + weight_norm + pos_loss_avg
    ...
    self.optimizer, grad_norm_total = util.apply_clipped_optimizer(
        opt_fcn, self.loss)
...
class trainModel:
  def __init__(self, model, train_paras, trainset, testset, flgEval=False):
    ...
    self.testIter = train_paras.get(
        'testIter', 50)  # Print validation every testIter iterations
    ...
    if self.testset is not None:
      ...
      self.nBatchTest = max(
          1,
          np.ceil(self.testset._dataSize / self.batchSize).astype('int32'))
  ...
  def run(self):
    ...
    for itr in range(self.nIter):
      trn_loss_avg, acc_trn_avg, acc_trn_single_avg = self._trainEpoch(
          sess, itr, trn_loss_avg, acc_trn_avg, acc_trn_single_avg)
      lsTrainAcc.append(acc_trn_avg)

      if (itr % self.testIter == 0) & (self.flgTest):
        val_loss_avg, acc_val_avg, pred = self._test(sess)
        print('itr:', itr, 'trn_loss', round(trn_loss_avg, 4), 'val_loss',
              round(val_loss_avg, 4))
        print('itr:', itr, 'trn_acc', round(acc_trn_avg, 4), 'trn_single_acc',
              round(acc_trn_single_avg, 4), 'val_acc', round(acc_val_avg, 4))
  ...
  def _trainEpoch(self, sess, iter, trn_loss_avg, acc_trn_avg,
                  acc_trn_single_avg):
    cmd, pos, act, mask, struct, cmd_length, idx = self.trainset.next_batch(
        self.batchSize, isTrain=True)
    trn_feed_dict = {
        self.m.cmd_ind: cmd,
        self.m.pos_ind: pos,
        self.m.act_ind: act,
        self.m.mask_ph: mask,
        self.m.act_lengths: np.clip(struct, a_min=1, a_max=None),
        self.m.cmd_lengths: cmd_length
    }
    trn_feed_dict[self.m.learning_rate] = self.lrInit / (np.power(
        iter + 10, .6))
    _, trn_loss, acc_trn_single, acc_trn = sess.run([
        self.m.optimizer, self.m.loss, self.m.percent_correct,
        self.m.percent_fully_correct
    ], trn_feed_dict)
  ...
  def _test(self, sess):
    ...
    for i in range(self.nBatchTest):
      cmd, pos, act, mask, struct, cmd_length, idx = self.testset.next_batch(
          self.batchSize, isTrain=False)
      val_feed_dict = {
          self.m.cmd_ind: cmd,
          self.m.pos_ind: pos,
          self.m.act_ind: act,
      ...
      val_loss, acc_val, pred_i = sess.run(
          [self.m.loss, self.m.percent_fully_correct, self.m.logprobabilities],
          val_feed_dict)
      bs = len(cmd)
